...
public class PacketAnalyzer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length == 0) {
            System.out.println("⚠️ No arguments provided. Using hardcoded defaults: sim 10 HTTP,DNS output.txt");
            args = new String[] {
                    "sim", "10", "HTTP,DNS", "output.txt"
            };
        }
    ...
    // ============================================================
    public record PcapPacketStub(long frameNo, byte[] raw, Instant ts, int origLen, int capLen) {}
    ...
    // ============================================================
    // CAPTURE SERVICES (Live & Simulated)
    // ============================================================
    public static final class PcapCaptureService implements CaptureService {
        ...
        public void startCapture(PacketHandler handler, int maxSeconds) throws Exception {
            PacketListener pl = packet -> {
                ...
                PcapPacketStub stub = new PcapPacketStub(frameNo, raw, timestamp, raw.length, raw.length);

                boolean keepGoing = handler.handle(stub);
                if (!keepGoing) {
                    try { handle.breakLoop(); } catch (NotOpenException ignored) { }
                }
            };

            try { handle.loop(-1, pl); } catch (InterruptedException e) { Thread.currentThread().interrupt(); }
        }
    ...
    public static final class SimulatedCaptureService implements CaptureService {
        ...
        private static void sleepForSimulation() {
            try {
                Thread.sleep(200);
        ...
        private static byte[] makeEthernetIpv4TcpPacket(byte[] payload, int sport, int dport) {
            ...
            eth[6] = 0x00; eth[7] = 0x11; eth[8] = 0x22; eth[9] = 0x33; eth[10] = 0x44; eth[11] = 0x55;
            eth[12] = 0x08; eth[13] = 0x00;
            byte[] ip = new byte[20];
            ip[0] = 0x45; ip[1] = 0x00;
            int totalLen = ip.length + 20 + payload.length;
            ip[2] = (byte) ((totalLen >> 8) & 0xff); ip[3] = (byte) (totalLen & 0xff);
            ip[8] = 64; ip[9] = 6;
            ip[12] = (byte) 192; ip[13] = (byte) 168; ip[14] = 0; ip[15] = 2;
            ip[16] = (byte) 142; ip[17] = (byte) 250; ip[18] = (byte) 193; ip[19] = (byte) 110;
            byte[] tcp = new byte[20];
            tcp[0] = (byte) ((sport >> 8) & 0xff); tcp[1] = (byte) (sport & 0xff);
            tcp[2] = (byte) ((dport >> 8) & 0xff); tcp[3] = (byte) (dport & 0xff);
            tcp[12] = 0x50; tcp[13] = 0x18;
            tcp[14] = 0x00; tcp[15] = 0x64;
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            try { baos.write(eth); baos.write(ip); baos.write(tcp); baos.write(payload); } catch (IOException ignored) {}
        ...
        private static byte[] makeEthernetIpv4UdpPacket(byte[] payload, int sport, int dport) {
            ...
            ip[8] = 64; ip[9] = 17;
            ip[12] = (byte) 192; ip[13] = (byte) 168; ip[14] = 0; ip[15] = 2;
            ip[16] = (byte) 8; ip[17] = (byte) 8; ip[18] = 8; ip[19] = 8;
            byte[] udp = new byte[8];
            udp[0] = (byte) ((sport >> 8) & 0xff); udp[1] = (byte) (sport & 0xff);
            udp[2] = (byte) ((dport >> 8) & 0xff); udp[3] = (byte) (dport & 0xff);
            udp[4] = (byte) (((8 + payload.length) >> 8) & 0xff); udp[5] = (byte) ((8 + payload.length) & 0xff);
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            try { baos.write(eth); baos.write(ip); baos.write(udp); baos.write(payload); } catch (IOException ignored) {}
            return baos.toByteArray();
        }

        private static byte[] buildSyntheticDnsQuery(String qname) {
            ByteArrayOutputStream b = new ByteArrayOutputStream();
            b.write(0x12); b.write(0x34); b.write(0x01); b.write(0x00);
            b.write(0x00); b.write(0x01); b.write(0x00); b.write(0x00);
            b.write(0x00); b.write(0x00); b.write(0x00); b.write(0x00);
            for (String label : qname.split("\\.")) {
                b.write(label.length());
                try { b.write(label.getBytes(StandardCharsets.US_ASCII)); } catch (IOException ignored) {}
            }
            b.write(0x00); b.write(0x00); b.write(0x01); b.write(0x00); b.write(0x01);
            return b.toByteArray();
        }

        // Extracted method to build the core TLS Client Hello body (hBytes)
        private static byte[] buildTlsClientHelloBody(String sni) throws IOException {
            ByteArrayOutputStream hello = new ByteArrayOutputStream();
            hello.write(0x03); hello.write(0x03);
            for (int i = 0; i < 32; i++) hello.write(0);
            hello.write(0);
            hello.write(0x00); hello.write(0x02);
            hello.write(0x00); hello.write(0x2f);
            hello.write(0x01);
            hello.write(0x00);

            byte[] host = sni.getBytes(StandardCharsets.US_ASCII);
            ByteArrayOutputStream sniExt = new ByteArrayOutputStream();
            sniExt.write(0x00); sniExt.write(0x00);
            sniExt.write((byte) ((host.length + 5) >> 8)); sniExt.write((byte) ((host.length + 5) & 0xff));
            sniExt.write(0x00);
            sniExt.write((byte) ((host.length + 3) >> 8)); sniExt.write((byte) ((host.length + 3) & 0xff));
            sniExt.write(0x00);
            sniExt.write((byte) ((host.length) >> 8)); sniExt.write((byte) (host.length & 0xff));
            sniExt.write(host);

            byte[] ext = sniExt.toByteArray();
            hello.write((byte) ((ext.length >> 8) & 0xff)); hello.write((byte) (ext.length & 0xff));
            hello.write(ext);
        ...
        private static byte[] buildSyntheticTlsClientHello(String sni) {
            ByteArrayOutputStream b = new ByteArrayOutputStream();
            try {
                b.write(0x16); b.write(0x03); b.write(0x03);
                ByteArrayOutputStream body = new ByteArrayOutputStream();
                body.write(0x01);
                byte[] hBytes = buildTlsClientHelloBody(sni);
        ...
        public void startCapture(PacketHandler handler, int maxSeconds) {
            String[] domains = {"example.com", "google.com", "bing.com"};
            int[] clientPorts = {12345, 51322, 60000};
            int httpCount = 0;
            int dnsCount = 0;
            int tlsCount = 0;
            ...
            for (int i = 0; i < total && !closed; i++) {
                byte[] raw;
                int domainIndex = i % domains.length;
                int portIndex = i % clientPorts.length;
                String domain = domains[domainIndex];
                int clientPort = clientPorts[portIndex];

                if (i % 3 == 0) {
                    byte[] httpPayload = ("GET /" + ++httpCount + ".html HTTP/1.1\r\nHost: " + domain + "\r\n\r\n").getBytes(StandardCharsets.US_ASCII);
                    raw = makeEthernetIpv4TcpPacket(httpPayload, clientPort, 80);
                } else if (i % 3 == 1) {
                    byte[] dnsPayload = buildSyntheticDnsQuery("query" + ++dnsCount + "." + domain);
                    int dnsDport = (i % 7 == 0) ? 5353 : 53;
                    raw = makeEthernetIpv4UdpPacket(dnsPayload, clientPort, dnsDport);
                } else {
                    byte[] tlsPayload = buildSyntheticTlsClientHello("secure" + ++tlsCount + "." + domain);
                    raw = makeEthernetIpv4TcpPacket(tlsPayload, clientPort, 443);
                }
    ...
    // ============================================================
    // 1. PHYSICAL & FRAME LAYER PARSERS (L1)
    // ============================================================
    public static final class PhysicalParser implements LayerParser {
        private final String iface;

        public PhysicalParser(String iface) {
            this.iface = iface;
        }
        ...
        public void parse(PacketContext ctx) {
            ...
            m.put("LinkType", ctx.pkt.raw.length >= 14 ? "Ethernet" : "Unknown");
            ctx.putLayer("PHYSICAL", m);
        }
    }
    ...
    public static final class LinkParser implements LayerParser {
        private static String mac(byte[] b, int off) {
            ...
            for (int i = 0; i < 6; i++) {
                macParts[i] = String.format("%02x", b[off + i]);
            }
    ...
    // ============================================================
    // 5. APPLICATION LAYER PARSERS (L5-L7)
    // ============================================================
    ...
    public static final class HttpParser implements ApplicationParser {
        public boolean canParse(PacketContext ctx, TransportInfo t) {
            ...
            if (ctx.pkt.raw.length < 54) return false;
            String s = new String(ctx.pkt.raw, 54, ctx.pkt.raw.length - 54, StandardCharsets.US_ASCII);
        ...
        public Map<String, String> parse(PacketContext ctx, TransportInfo t) {
            ...
            int ipStart = 14;
            int ihl = (r[ipStart] & 0x0f) * 4;
            int payloadStartOffset = ipStart + ihl + 20;

            if (r.length <= payloadStartOffset) {
                m.put("AppProto", "HTTP");
                m.put("FirstLine", "No Payload");
                return m;
            }

            String ascii = new String(r, payloadStartOffset, r.length - payloadStartOffset, StandardCharsets.US_ASCII);
            ascii = ascii.replaceAll("[^\\p{Print}\\s]", "").trim();
            int idx = ascii.indexOf("\r\n");
            m.put("AppProto", "HTTP");
            String firstLine = idx > 0 ? ascii.substring(0, idx) : ascii;
            m.put("FirstLine", firstLine.isEmpty() ? "Partial/Empty Payload" : firstLine);
            return m;
        }
    }
    ...
    public static final class DnsParser implements ApplicationParser {
        public boolean canParse(PacketContext ctx, TransportInfo t) {
            ...
            return (t.sport == 53 || t.dport == 53 || t.dport == 5353) && ctx.pkt.raw.length >= 42;
        }
    ...
    public static final class TabRowPacketFormatter implements PacketFormatter {
        // Helper to convert a layer map to its tab-separated row string
        private String formatLayer(String layerName, Map<String, String> fields) {
            List<String> fieldsList = fields.entrySet().stream()
                    .map(x -> x.getKey() + "=" + x.getValue())
                    .collect(Collectors.toCollection(ArrayList::new));

            fieldsList.add(0, layerName);
            return String.join("\t", fieldsList);
        }

        public List<String> format(PacketContext ctx) {
            List<String> rows = new ArrayList<>();
            Map<String, String> layerRows = new HashMap<>();

            // 1. Convert all layers to their final string representation
            for (Map.Entry<String, Map<String, String>> e : ctx.layers.entrySet()) {
                layerRows.put(e.getKey(), formatLayer(e.getKey(), e.getValue()));
            }

            // 2. Construct the final list in the required sequence: FRAME, empty line, others
            if (layerRows.containsKey("FRAME")) {
                rows.add(layerRows.get("FRAME"));
            }
            rows.add(""); // Empty line separator

            // Add remaining layers in sequence
            if (layerRows.containsKey("PHYSICAL")) {
                rows.add(layerRows.get("PHYSICAL"));
            }
            if (layerRows.containsKey("LINK")) {
                rows.add(layerRows.get("LINK"));
            }
            if (layerRows.containsKey("NETWORK")) {
                rows.add(layerRows.get("NETWORK"));
            }
            if (layerRows.containsKey("TRANSPORT")) {
                rows.add(layerRows.get("TRANSPORT"));
            }
            if (layerRows.containsKey("APP")) {
                rows.add(layerRows.get("APP"));
            }

            // Replaced '----' with a full-line separator
            rows.add("================================================================================");
            return rows;
        }
    }
    ...
    public static final class SequentialConsoleOutputWriter implements OutputWriter {
        private final ConcurrentSkipListMap<Long, List<String>> queue = new ConcurrentSkipListMap<>();
        private final AtomicLong nextSeq = new AtomicLong(1);

        public void writeRows(List<String> rows) {
            String frameRow = rows.get(0); // FRAME is now the first row
            // We need to carefully parse the FRAME row since it is the first row.
            // FRAME\tFrameNo=1\t...
            // Extract 'FrameNo=1' part, which is the second element (index 1) after splitting by tab.
            String frameNoField = frameRow.split("\t")[1];
            // Extract '1' part after splitting by '='
            long seq = Long.parseLong(frameNoField.split("=")[1]);

            queue.put(seq, rows);
            checkAndPrint();
        }

        private synchronized void checkAndPrint() {
            while (!queue.isEmpty() && queue.firstKey() == nextSeq.get()) {
                List<String> rows = queue.pollFirstEntry().getValue();
        ...
        public synchronized void flushQueue() {
            if (!queue.isEmpty()) {
                System.out.println("--- Flushing remaining queue (Out-of-order stragglers) ---");
                while (!queue.isEmpty()) {
                    queue.pollFirstEntry().getValue().forEach(System.out::println);
                }
            }
        }
    }
    ...
    public static final class PacketAnalyzerApp {
        ...
        public PacketAnalyzerApp(Config cfg) {
            ...
            this.writer = (cfg.outFile() != null)
                    ? new FileOutputWriter(cfg.outFile())
                    : new SequentialConsoleOutputWriter();

            List<LayerParser> parsers = List.of(
                    new PhysicalParser(cfg.iface()),
                    new FrameParser(),
                    new LinkParser(),
                    new NetworkParser(),
                    new TransportParser()
        ...
        public void run() throws Exception {
            // --- OUTPUT HEADER START ---
            System.out.println("===============================================================================================");
            System.out.println("Concurrent Packet Analyzer Report");
            System.out.printf("Started: %s | Interface/Mode: %s | Duration: %ds | Protocols: %s%n",
                    Instant.now().atZone(ZoneOffset.systemDefault()), cfg.iface(), cfg.durationSec(), cfg.protocols().stream().collect(Collectors.joining(",")));
            System.out.println("===============================================================================================");
            // --- OUTPUT HEADER END ---
            ...
            try (capture) {
                ...
                if (writer instanceof SequentialConsoleOutputWriter scow) {
                    scow.flushQueue();
                }
    ...
    // ============================================================
    // UTILITY: STRUCTURED BATCH EXECUTOR
    // ============================================================
